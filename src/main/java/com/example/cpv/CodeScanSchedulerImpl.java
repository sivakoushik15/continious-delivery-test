...
public class CodeScanSchedulerImpl implements Scheduler, ProcessEventListener, ProcessLifecycleListener, AppStateListener {

  private static final Logger LOG = LoggerFactory.getLogger(CodeScanSchedulerImpl.class);
  ...
  public CodeScanSchedulerImpl(AppSettings settings, AppReloader appReloader, CommandFactory commandFactory,
  ...
  CodeScanSchedulerImpl setProcessWatcherDelayMs(long l) {
  ...
  private void tryToStartEs() {
    ...
    if (process != null) {
        ...
        process.start(new Supplier<ProcessMonitor>() {
        	@Override
        	public ProcessMonitor get() {
        		return mon;
        	}
		});
    }
